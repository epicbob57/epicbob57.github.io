var channels = ['grandpoobear'], // Channels to initially join
...
var chat = document.getElementById('chat'),
	...
	client = new window.tmi.client(clientOptions);
	...
	for(var i in emotes) {
		...
		for(var j in e) {
			...
			if(typeof mote == 'string') {
				...
				splitText.splice(mote[0], 1, '<div class="tooltip"><span class="tooltiptext tooltip-bottom">'+i+'</span><img class="emoticon" src="http://static-cdn.jtvnw.net/emoticons/v1/' + i + '/3.0"></div>');
			}
		}
	}
...
client.addListener("subscription", function (channel, username, method, message, userstate) {
	...
	chatNotice(username + ' just subscribed with ' + method, null, null, 'chat-hosting-no');
