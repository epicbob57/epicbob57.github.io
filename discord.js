...
client.on('message', function(user, userID, channelID, message, event) {
	if(channelID === currentChannel.id)
  	document.getElementById("messages").insertAdjacentHTML("afterbegin",`<p><b>${user}#${event.d.author.discriminator}</b> ${message}</p>`);
});
...
function channelChange() {
  ...
  client.getMessages({
    channelID: currentChannel.id}, function(error, messages) {
